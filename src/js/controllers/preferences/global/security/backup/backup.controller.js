  ...
  function BackupCtrl($rootScope, $scope, $timeout, profileService, go, gettextCatalog, confirmDialog, notification, $log,
    ...
    const viewContentLoaded = function () {
      ...
      if (vm.isCordova) {
        ...
        Zip = require('zip');
        vm.text = gettextCatalog.getString(`To restore your wallets, you will need a full backup of TravelFlex data at ${appDataDir}.  
    ...
    function saveFile(file, cb) {
      const backupFilename = `TravelFlex${Date.now()}.encrypted`;
      if (!vm.isCordova) {
        const inputFile = document.getElementById('nwExportInputFile');
